...
h = 0          #step size, 0.02 good. if 0, program will set h automaticaly
o = 'sd'            #'cg' or 'sd'
####################
...
# automatically set step size
if not h:
    xlen = max(X_PES)-min(X_PES)
    ylen = max(Y_PES)-min(Y_PES)
    h = max([xlen,ylen]) / 1000.0 #default 1000
